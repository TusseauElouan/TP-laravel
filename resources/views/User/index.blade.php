...
<h1 class="font-bold text-center text-5xl m-4">Liste des utilisateurs</h1>
        ...
        <tbody>
            @forelse ($users as $user)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4">
                        <a href="{{ route('user.show', $user) }}">Voir détails</a>
                        <a href="{{ route('user.edit', $user)}}" class="ml-2 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-1 px-3 rounded-lg transition duration-200 ease-in-out">Modifier</a>
                    </td>
                </tr>
            @empty
            <tr>
...
<div class="max-w-3xl mx-auto bg-white rounded-lg shadow-lg p-8 mt-8">
    <h1 class="font-bold text-center text-5xl mb-6 text-gray-800">Détail de l'utilisateur</h1>
    ...
    <!-- Absences -->
    <div class="mt-8">
        <span class="font-semibold text-lg text-gray-700">Absences:</span>
                ...
                @foreach ($absences as $absence)
                    @if ($absence->user_id_salarie == Auth::user()->id)
                        @if($absence->isValidated)
                            <li class="text-green-600">Motif : {{ $absence->motif->libelle}} / Débute : {{$absence->date_absence_debut}} / Fini : {{$absence->date_absence_fin}}</li>
                        @else
                            <li class="text-red-600">Motif : {{ $absence->motif->libelle}} / Débute : {{$absence->date_absence_debut}} / Fini : {{$absence->date_absence_fin}}</li>
                        @endif
                    @endif
                @endforeach
            @endif
