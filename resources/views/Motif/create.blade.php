...
<div class="grid place-content-center text-center h-screen">
    <form method="POST" action="{{ route('motif.store') }}" class="flex flex-col border-gray-300 border-2 rounded-md space-y-6 p-10 w-80">
        ...
        <div class="flex items-center justify-center space-x-2">
            <input type="checkbox" name="is_accessible_salarie" id="accessible_salarie" class="w-6 h-6 text-indigo-600 bg-gray-200 border-gray-300 rounded focus:ring-blue-500 focus:ring-2 focus:ring-offset-2 focus:outline-none focus:ring-opacity-50" {{ old('is_accessible_salarie') ? 'checked' : '' }}>
